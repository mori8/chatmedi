      ...
      <div className="flex-1 overflow-auto">
        {status === "loading" ? (
          ...
          chatIds.length > 0 ? (
            <ul>
              {chatIds.map((_chatId) => (
                <li key={_chatId} className={classNames("py-2 px-3 truncate hover:bg-white hover:bg-opacity-20 rounded-xl", {
                  "bg-white bg-opacity-20": _chatId === chatId,
                })}>
                  <a href={`/chat/${_chatId}`} className="text-sm font-semibold">{`${_chatId}`}</a>
                </li>
              ))}
            </ul>
          ) : (
